...
train_data = data[:170]
target_train = target[:170]
val_data = data[170:]
target_val = target[170:]
...
def add_layer(inputs, in_size, out_size, n_layer, activation_function=None):
    ...
    return outputs, reg, Wx_plus_b
    #probability, l2_loss ,logit

h1 = 8
h2 = 8

layer1, reg1, W1 = add_layer(xt, 9, h1, n_layer=1, activation_function=tf.nn.relu)
layer2, reg2, W2 = add_layer(layer1, h1, h2, n_layer=2, activation_function=tf.nn.relu)
prediction, reg3, logit = add_layer(layer2, h2, 6, n_layer=3, activation_function=tf.nn.softmax)

with tf.name_scope('cost'):
    cost = (tf.losses.softmax_cross_entropy(onehot_labels=yt, logits=logit, scope='Cost_Function')
            + 0.05*reg1 + 0.05*reg2 + 0.05*reg3)
...
with tf.Session() as sess:
    ...
    print(sess.run(accuracy, feed_dict={xt: feat_val, yt: target_val}))
'''
Open tensorboard
